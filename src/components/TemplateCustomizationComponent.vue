<template>
  <div style="position: relative">
    ...
    >
      ...
      >
        <q-tab v-for="(sectionName) in Object.keys(variablesJson)" :key="sectionName" :name="sectionName" :label="sectionName" />
      </q-tabs>

      <!-- Section Bodies -->
      <q-tab-panels v-model="state.customizeTab" animated>
        <q-tab-panel v-for="[sectionName, variables] in Object.entries(variablesJson)" :key="sectionName" :name="sectionName">
          <q-list bordered>
            <q-item v-for="[variableName, props] in Object.entries(variables)" :key="variableName">
              ...
              <q-item-section>
                ...
                <template v-if="props.type === 'color'">
                  <q-input v-model="variablesJson[sectionName][variableName].value" :rules="[(val) => testPattern.hexOrHexaColor(val) || 'Invalid color']" :hint="props.hint" filled>
                    <template v-slot:prepend>
                      <q-icon name="square" class="cursor-pointer" :style="{ color: variablesJson[sectionName][variableName].value }">
                        <q-popup-proxy
                          cover
                          transition-show="scale"
                          transition-hide="scale"
                        >
                          <q-color v-model="variablesJson[sectionName][variableName].value" />
                        </q-popup-proxy>
                      </q-icon>
                    </template>
                  </q-input>
                ...
                <template v-else-if="props.type === 'image'">
                  ...
                  <q-item-label caption class="q-pt-md">{{ props.hint }}</q-item-label>
                </template>

                <!-- Text Area Input -->
                <template v-else-if="props.type === 'text'">
                  <q-input v-model="variablesJson[sectionName][variableName].value" type="textarea" :hint="props.hint" filled />
                </template>

                <!-- Other (Show as generic input) -->
                <template v-else>
                  <q-input v-model="variablesJson[sectionName][variableName].value" :hint="props.hint" filled />
                </template>
              </q-item-section>
            </q-item>
          </q-list>
...
function onFileChange(newFile: File, sectionName: string, variableName: string) {
  if (!newFile) return
