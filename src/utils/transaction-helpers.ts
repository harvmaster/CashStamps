import { AddressGetHistory, AddressListUnspent } from 'src/services/electrum-types'
...
export const getAddresses = (parentNode: HDPrivateNode, offset = 0, count = ADDRESS_GAP) => {
...
export const getUsedKeys = async (parentNode: HDPrivateNode, offset = 0, count = ADDRESS_GAP): Promise<KeyHistory[]> => {
  const addresses = getAddresses(parentNode, offset, count);

  const historyPromises: Promise<KeyHistory>[] = addresses.map(async ({ address, node }) => {

    // Get the history of the key from electrum
    const res = await app.electrum.request('blockchain.address.get_history', address) as AddressGetHistory['response']
    return {
      history: res,
      node,
      address
  ...
  if (history.every((key) => key.history.length)) {
    history.push(...await getUsedKeys(parentNode, offset + ADDRESS_GAP, count))
  }
...
export const getTransactionData = async (key: KeyHistory) => {
  const res = await app.electrum.request('blockchain.transaction.get', key.history[0].tx_hash, true) as any
  return res
}

export const getTransactionBlocktime = async (key: KeyHistory): Promise<number> => {
